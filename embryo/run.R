...
p=num_cut
q=15
final=matrix(NA,3,14)
KMat=matrix(NA,3,7)

a=readRDS("counts.Rds")
b=readRDS("metadata.Rds")
...
n=rep(NA,3)
for (i in 1:3){
  ID=which(b$embryo==paste("embryo",i,sep=""))
  part_b=b[ID,] #cell type
  part_a=a[,ID] #count
  
  na=which(part_b$celltype_mapped_refined=="Low quality")
  part_b=part_b[-na,]
  part_a=part_a[,-na]
  
  data[[i]]=t(apply(part_a, 2, process_column))#     t(part_a)
  position[[i]]=cbind(part_b$x_global,part_b$y_global)
  label[[i]]=part_b$celltype_mapped_refined
  n[i]=ncol(part_a)
}
...
for (i in 1:3){
  ...
  #top_columns <- colnames(Xst)[order(apply(Xst, 2, var), decreasing = TRUE)[1:p]]
  #Xst <- Xst[, top_columns]
  Xsc=do.call(rbind, data[-i])
  ...
  res_drsc_K <- DR.SC(seu, q=q,K=20:30, platform = "Visium",  verbose=FALSE)
  ...
  res_gmm0_K=Mclust(hZ, G=20:30, modelNames='EEE',verbose=FALSE)
  ...
  set.seed(seed+i)
  scmeb_K = SC.MEB(hZ, Adj_sp, beta_grid = seq(0.5, 5, by=0.5), K_set= 20:30, parallel=TRUE, num_core = 3, PX = TRUE)
  out2 = SC.MEB::selectK(scmeb_K, K_set = 20:30,criterion = "BIC")
  ...
  ####select K
  res=trans_k_bic(q=q,K_set=20:30,Ux=cbind(hZ,pos),zeta=zeta,method="HBIC",verbose=FALSE)
  K_ours=res$K
  ...
  set.seed(seed+i)
  res2=TransST3(Xsc=Xsc_new,z_sc=z_sc,Xst=raw,q=q,K=K,nS=n_st,maxIter=100,zeta=zeta,combine=FALSE,neighbor=neighbor,weighted=weighted,verbose=FALSE)
  ...
  if(K_ours==K){
    ...
    set.seed(seed+i)
    res2_K=TransST3(Xsc=Xsc_new,z_sc=z_sc,Xst=raw,q=q,K=K_ours,nS=n_st,maxIter=100,zeta=zeta,combine=FALSE,neighbor=neighbor,weighted=weighted,verbose=FALSE)
  }
  
  #########
  KMat[i,]=abs(KMat[i,]-K)
  
  #final[i,1:2]=c(adjustedRandIndex(res2$z, z_st),adjustedRandIndex(res2_K$z, z_st))
